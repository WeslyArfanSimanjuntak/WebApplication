...
function ServerOData(sUrl, aoData, fnCallback, oSettings) {
    ...
    $.each(oSettings.aoColumns, function (i, value) {
        var sFieldName = null;
        if ((value.oData !== null && value.oData !== "" && value.oData !== undefined)) {
            sFieldName = value.oData;
        } else {
            sFieldName = (value.sName !== null && value.sName !== "") ? value.sName : ((typeof value.mData === 'string') ? value.mData : null);
        }
        ...
        if (data.$select === null || data.$select === undefined) {
            data.$select = sFieldName;
        } else {
            //<wesly 3/3/2019>
            var arrFieldName = sFieldName.split(';');
            $.each(arrFieldName, function (ii, valuee) {
                data.$select += "," + valuee;
            });
            //<\wesly 3/3/2019>
            //data.$select += "," + sFieldName;
    ...
    if (oSettings.oFeatures.bServerSide) {
        ...
        $.each(oSettings.aoColumns,
            function (i, value) {
                ...
                var sFieldName = value.oData || value.sName || value.mData;
                                ...
                                (value.sType === 'numeric') ?
                                    ...
                                    function (val) {
                                        ...
                                        switch (oSettings.oInit.iODataVersion) {
                                            ...
                                            case 3: return "datetime'" + (new Date(val)).toISOString() + "'";
                                            // V2 works with the following format:
